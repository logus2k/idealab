...
| ID | Task | Status | Output | Depends on |
|---|---|---|---|---|
| T2.1 | **ID convention for models / datasets in `links.json`** — HF id strings directly (not UUIDs). Documented in `data/README.md`. | ✅ | `data/README.md` written (file & link table, owner column, justification for HF id strings). | — |
| T2.2 | **`idea_models` curation** — task-level starter set (TASK_TO_MODELS map per HF task slug), propagated to each idea via its `idea_tasks` edges. Coarse but functional; trivially overridable per-idea. | ✅ | `scripts/curate_tier_b.py` written + run. **228 ideas with model suggestions** (avg 3.7 each). idea_models SQL table + indexes added. | T1.3, T2.1 |
| T2.3 | **`idea_datasets` curation** — same TASK_TO_DATASETS pattern. | ✅ | **220 ideas with dataset suggestions**. idea_datasets SQL table + indexes added. | T2.1 |
| T2.4 | **Plans registry** — `data/plans.json` auto-generated from `plans/index.json` + each plan's frontmatter. UUIDs via `uuidgen`, never regenerated on re-run. | ✅ | `scripts/build_plans_registry.py` written + run. **25 plans** registered (8 case-study, 6 consultancy, 6 industry, 5 vendor). | — |
| T2.5 | **`plan_ideas` + `plan_models` + `plan_datasets`** — `plan_ideas` derived via tag-overlap-score (same logic the UI's "Related ideas" widget uses). `plan_models` / `plan_datasets` derived as the consensus union of idea_models / idea_datasets for the plan's top ideas (model appears in ≥2 of the plan's ideas). | ✅ | All 25 plans now have idea+model+dataset links. plan_models / plan_datasets SQL tables + indexes added. | T2.4, T2.2, T2.3 |
| T2.5b | **`plan_requirements` + `plan_kpis` curation** — derive from plan_ideas → idea_requirements / idea_kpis (consensus filter, similar to plan_models). Currently NOT populated. | ⬜ | Pending follow-up | T2.5 |
